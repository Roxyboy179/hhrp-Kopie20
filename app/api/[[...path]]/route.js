...
export async function POST(request) {
  ...
  switch (p) {
    ...
    case 'shop/purchase': return handlePurchaseItem(request);
    case 'shop/my-items': return handleGetMyItems(request);
    case 'profile/set-decoration': return handleSetDecoration(request);
...
async function handleGetShopItems(request) {
  try {
    ...
    const items = Object.values(SHOP_ITEMS).filter(item => {
      if (item.betaOnly && !isBetaTester) return false;
      return true;
    });
...
async function handlePurchaseItem(request) {
  try {
    ...
    // Check beta only items
    if (item.betaOnly && !user.roles?.includes('1494434149623136276')) {
      return NextResponse.json({ error: 'Nur für Beta Tester' }, { status: 403 });
    }
    ...
    const ownedItems = userDataObj.owned_items || [];
    const bankBalance = userDataObj.bank || 0;

    // Check if already owned
    if (ownedItems.includes(itemId)) {
      return NextResponse.json({ error: 'Item bereits gekauft' }, { status: 400 });
    }

    // Check if VIP (all items free for VIP)
    const licenses = userDataObj.licenses || [];
    const isVIP = licenses.some(l => l.startsWith('vip_'));

    let finalPrice = item.price;
    if (isVIP && item.vipFree) {
      finalPrice = 0;
    }

    // Check balance
    if (finalPrice > 0 && bankBalance < finalPrice) {
      return NextResponse.json({ 
    ...
    const newBalance = bankBalance - finalPrice;
    const newOwnedItems = [...ownedItems, itemId];

    userDataObj.bank = newBalance;
    userDataObj.owned_items = newOwnedItems;
    ...
    return NextResponse.json({ 
      success: true,
      message: finalPrice === 0 ? 'Item kostenlos erhalten!' : 'Item gekauft!',
      newBalance,
      item
    });
    ...
    return NextResponse.json({ error: 'Kauffehler' }, { status: 500 });
  }
}

// Gekaufte Items abrufen
async function handleGetMyItems(request) {
  try {
    const user = getUserFromRequest(request);
    if (!user) {
      return NextResponse.json({ error: 'Nicht authentifiziert' }, { status: 401 });
    ...
    const userDataObj = typeof userData?.data === 'string' ? JSON.parse(userData.data) : (userData?.data || {});
    const ownedItems = userDataObj.owned_items || [];
    const activeDecoration = userDataObj.active_decoration || null;
    const bankBalance = userDataObj.bank || 0;
    ...
    console.error('Get my items error:', e);
    return NextResponse.json({ error: 'Fehler' }, { status: 500 });
  }
}

// Aktive Dekoration setzen
async function handleSetDecoration(request) {
  try {
    const user = getUserFromRequest(request);
    if (!user) {
      return NextResponse.json({ error: 'Nicht authentifiziert' }, { status: 401 });
    ...
    // Check if owned (or null to remove)
    if (itemId && !ownedItems.includes(itemId)) {
      return NextResponse.json({ error: 'Item nicht gekauft' }, { status: 400 });
    }

    // Set active decoration
    userDataObj.active_decoration = itemId;
    ...
    return NextResponse.json({ 
      success: true,
      activeDecoration: itemId
    });

  } catch (e) {
    console.error('Set decoration error:', e);
    return NextResponse.json({ error: 'Fehler' }, { status: 500 });
  }
}
