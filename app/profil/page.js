...
import { 
  ...
  Gamepad2, Shield, TrendingUp, Heart, Scale, Siren, Eye, Download
} from 'lucide-react';
...
export default function HomePage() {
  ...
  const [websiteStats, setWebsiteStats] = useState({ totalVisits: 0, uniqueVisitors: 0, appInstalls: 0 });
    ...
    <div className="min-h-screen relative">
      ...
      <section className="px-4 sm:px-6 py-16 md:py-24">
        <div className="max-w-5xl mx-auto">
          ...
          {/* Website Stats Grid - NEU */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
            {[
              { icon: <Eye className="w-5 h-5" />, value: websiteStats.totalVisits, label: 'Webseiten-Besuche', gradient: 'from-blue-500 to-cyan-500' },
              { icon: <Users className="w-5 h-5" />, value: websiteStats.uniqueVisitors, label: 'Einzigartige Besucher', gradient: 'from-purple-500 to-pink-500' },
              { icon: <Download className="w-5 h-5" />, value: websiteStats.appInstalls, label: 'App-Downloads', gradient: 'from-green-500 to-emerald-500' },
            ].map((stat, i) => (
              <GlassCard key={i} className="p-6 md:p-8 text-center animate-fade-in-up" hover>
                <div 
                  className={`w-12 h-12 md:w-14 md:h-14 rounded-xl flex items-center justify-center mx-auto mb-4 transition-all duration-300 bg-gradient-to-br ${stat.gradient}`}
                  style={{ 
