    ...
    >
      ...
      <div className="max-w-2xl w-full relative animate-[fade-in-up_0.6s_cubic-bezier(0.22,1,0.36,1)]">
        ...
        >
          ...
          {/* Action Buttons */}
          <div className="mt-7 flex flex-col sm:flex-row gap-3 sm:gap-3">
            {/* Refresh Button */}
            <button
              onClick={() => fetchStatus(true)}
              disabled={refreshing}
              className="w-full sm:flex-1 h-16 sm:h-12 px-12 sm:px-6 rounded-xl flex items-center justify-center gap-3 text-[15px] sm:text-[13px] font-semibold transition-all active:scale-[0.98] border disabled:opacity-60"
              style={{
            ...
            >
              <RefreshCw className="w-5 h-5 sm:w-4 sm:h-4 flex-shrink-0" style={refreshing ? { animation: 'spin 1s linear infinite' } : {}} />
