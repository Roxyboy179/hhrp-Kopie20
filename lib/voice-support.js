...
export default function VoiceSupportPage() {
  ...
  const startSession = async () => {
    // Außerhalb Support-Zeiten: Audio abspielen, KEINE Session erstellen
    if (!isSupportOpen()) {
      try {
        const el = closedAudioRef.current;
        if (el) {
          el.muted = false;
          el.currentTime = 0;
          el.play().catch(() => {
            toast.info('Support ist aktuell geschlossen (täglich 12:00–22:00 Uhr).');
          });
        } else {
          toast.info('Support ist aktuell geschlossen (täglich 12:00–22:00 Uhr).');
        }
  ...
  return (
    <div className="relative overflow-hidden rounded-[28px] border border-white/[0.08] bg-gradient-to-b from-white/[0.04] to-white/[0.01] backdrop-blur-2xl shadow-2xl">
      ...
      <div className="p-6 sm:p-10 space-y-6">
        ...
        {/* Support-Zeiten-Hinweis (nur wenn geschlossen) */}
        {!supportOpen && (
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-500/[0.08] via-purple-500/[0.06] to-indigo-500/[0.08] border border-indigo-500/[0.25] p-4 flex gap-3">
            ...
            <div className="text-xs sm:text-sm text-white/80 leading-relaxed space-y-1">
              ...
              <div>
                ...
                {openIn && (
                  <> Wir sind wieder <strong className="text-emerald-200">{openIn}</strong> für dich da.</>
                )}
              </div>
            </div>
          </div>
