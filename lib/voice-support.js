...
export async function sendVoiceSupportTranscript(session) {
  if (!session) return;
  try {
    // HTML generieren – falls der Builder explodiert, nicht den ganzen Flow killen
    let html = '';
    let filename = `voice-support-${session.id || 'unknown'}.html`;
    try {
      html = buildVoiceSupportTranscriptHtml(session);
      filename = voiceSupportTranscriptFilename(session);
    } catch (htmlErr) {
      console.error('[VoiceSupport] transcript html build failed:', htmlErr);
      html = `<!doctype html><meta charset="utf-8"><title>Voice Support Transcript</title>` +
             `<pre>Transcript-Generator-Fehler: ${String(htmlErr?.message || htmlErr)}</pre>` +
