// ═══════════════════════════════════════════════════════════════
// 🎉 SHOP-AKTIONEN / RABATT-KAMPAGNEN — Shared Logic
// ═══════════════════════════════════════════════════════════════
// Wird sowohl vom Frontend (ShopView, Landingpage) als auch vom
// Backend (/api/shop/purchase, /api/shop/gift) importiert, damit die
// Preisberechnung überall EXAKT gleich ist und der angezeigte
// Rabatt auch tatsächlich abgezogen wird.
//
// Neue Aktion anlegen: einfach Eintrag zu SHOP_PROMOTIONS hinzufügen.
//
// eligibility:
//   - 'non_vip'  → nur User OHNE aktives VIP oder Luxus-Pass
//   - 'all'      → alle User
//
// appliesTo:
//   - categories: Liste der SHOP_ITEMS.category, auf die der Rabatt greift
//   - itemIds:    (optional) konkrete Item-IDs
//
// discount:            Prozent als Dezimalzahl (0.25 = 25% Rabatt)
// startDate/endDate:   ISO-Date-Strings (inkl. Uhrzeit)
// ═══════════════════════════════════════════════════════════════

export const SHOP_PROMOTIONS = [
  {
    id: 'fuehrerschein_fruehjahr_2026',
    title: 'Frühjahrs-Aktion: Führerscheine',
    description: '25% Rabatt auf ALLE Führerscheine',
    iconName: 'Car',
    type: 'discount',
    discount: 0.25,
    startDate: '2026-04-20T00:00:00',
    endDate:   '2026-05-21T23:59:59',
    eligibility: 'non_vip',
    appliesTo: { categories: ['führerscheine'] },
    badgeLabel: '-25% Aktion',
    notEligibleReason: 'Nur für Nutzer ohne VIP oder Luxus-Pass',
  },
  {
    id: 'vip_fruehjahr_2026',
    title: 'Frühjahrs-Aktion: VIP Platinum & höher',
    description: '30% Rabatt auf VIP Platinum, Ultimate, Elite Plus und Luxus-Pass',
    iconName: 'Gem',
    type: 'discount',
    discount: 0.30,
    startDate: '2026-04-20T00:00:00',
    endDate:   '2026-05-21T23:59:59',
    eligibility: 'non_vip',
    appliesTo: {
      itemIds: ['vip_platinum', 'vip_ultimate', 'vip_elite_plus', 'luxus_pass'],
    },
    badgeLabel: '-30% Aktion',
    notEligibleReason: 'Nur für Nutzer ohne VIP oder Luxus-Pass',
  },
  {
    id: 'credit_bonus_fruehjahr_2026',
    title: 'Frühjahrs-Aktion: +35% Credits Bonus',
    description: '35% mehr Credits ab 200 Credits Kauf (z.B. 200 → 270 Credits)',
    iconName: 'Coins',
    type: 'credit_bonus',
    bonusPercent: 0.35,
    minCredits: 200,
    maxCredits: 2000,
    startDate: '2026-04-20T00:00:00',
    endDate:   '2026-05-21T23:59:59',
    eligibility: 'all', // gilt für alle User (auch VIP), da es ein Bonus ist, kein Preis-Rabatt
    appliesTo: { categories: ['credits'] },
    badgeLabel: '+35% Bonus',
    notEligibleReason: null,
  },
];

// Hilfsfunktion: prüft ob User qualifiziert
// userHighestVIP: -1 = kein VIP, 0+ = hat irgendeinen VIP/Luxus-Pass
const isUserEligible = (promo, userHighestVIP) => {
  if (!promo || !promo.eligibility) return true;
  if (promo.eligibility === 'all') return true;
  if (promo.eligibility === 'non_vip') return userHighestVIP < 0;
  return false;
};

// Aktions-Match für ein bestimmtes Item (für calculatePrice).
// `item` muss ein SHOP_ITEMS-Objekt sein (mit .category).
// `now` ist optional (Default = aktuelle Zeit) – nützlich für Tests.
// WICHTIG: Gibt nur 'discount'-Typ Promotions zurück (nicht 'credit_bonus')
export const findActivePromotion = (item, itemId, userHighestVIP, now = new Date()) => {
  if (!item) return null;
  for (const promo of SHOP_PROMOTIONS) {
    // Nur Discount-Promotions, keine Credit-Bonus
    if (promo.type !== 'discount') continue;
    
    const start = new Date(promo.startDate);
    const end = new Date(promo.endDate);
    if (now < start || now > end) continue;

    const inIds = promo.appliesTo?.itemIds?.includes(itemId);
    const inCats = promo.appliesTo?.categories?.includes(item.category);
    if (!inIds && !inCats) continue;

    if (!isUserEligible(promo, userHighestVIP)) continue;

    return promo;
  }
  return null;
};

// Aktive Aktion (ohne Item-Filter) für den Banner.
// Wenn `userHighestVIP` nicht übergeben wird (undefined), wird die
// Eligibility NICHT geprüft (hilfreich auf der Landingpage für Gäste).
export const getActiveBannerPromotion = (userHighestVIP, now = new Date()) => {
  const checkEligibility = typeof userHighestVIP === 'number';
  for (const promo of SHOP_PROMOTIONS) {
    const start = new Date(promo.startDate);
    const end = new Date(promo.endDate);
    if (now < start || now > end) continue;
    if (checkEligibility && !isUserEligible(promo, userHighestVIP)) continue;
    return promo;
  }
  return null;
};

// Liefert ALLE aktuell aktiven Aktionen für den Banner-Slider.
// VIPs sehen auch "non_vip" Promos im Banner, bekommen sie aber nicht beim Kauf.
// Gibt zusätzlich ein `isEligible` Flag zurück für jeden User.
export const getActiveBannerPromotions = (userHighestVIP, now = new Date()) => {
  const checkEligibility = typeof userHighestVIP === 'number';
  const result = [];
  for (const promo of SHOP_PROMOTIONS) {
    const start = new Date(promo.startDate);
    const end = new Date(promo.endDate);
    if (now < start || now > end) continue;
    
    // ALLE Promos zurückgeben (auch non_vip für VIPs)
    // Aber mit Flag, ob User berechtigt ist
    const promoWithEligibility = {
      ...promo,
      isEligible: checkEligibility ? isUserEligible(promo, userHighestVIP) : true
    };
    result.push(promoWithEligibility);
  }
  return result;
};

// Formatiert ein Datum für die Anzeige (DD.MM.YYYY)
export const formatPromoDate = (iso) => {
  const d = new Date(iso);
  const dd = String(d.getDate()).padStart(2, '0');
  const mm = String(d.getMonth() + 1).padStart(2, '0');
  return `${dd}.${mm}.${d.getFullYear()}`;
};

// Sucht eine aktive Credit-Bonus-Aktion für eine bestimmte Credit-Menge.
// Liefert { promo, bonusCredits, totalCredits } oder null.
// NEU: Berücksichtigt auch Credits-Pässe (Standard: +15%, Elite+: +25%)
// Credits-Pass Bonus hat VORRANG vor Promotions!
export const findActiveCreditBonus = (creditsAmount, userHighestVIP, userLicenses = [], now = new Date()) => {
  const amount = Number(creditsAmount) || 0;
  if (amount <= 0) return null;
  
  // Prüfe Credits-Pässe (haben Vorrang!)
  const hasElitePlus = userLicenses.some(l => 
    (typeof l === 'string' && l === 'credits_elite_plus_pass') ||
    (typeof l === 'object' && l.id === 'credits_elite_plus_pass' && (!l.expiresAt || new Date(l.expiresAt) > now))
  );
  
  const hasStandard = userLicenses.some(l => 
    (typeof l === 'string' && l === 'credits_standard_pass') ||
    (typeof l === 'object' && l.id === 'credits_standard_pass' && (!l.expiresAt || new Date(l.expiresAt) > now))
  );
  
  if (hasElitePlus) {
    const bonusCredits = Math.floor(amount * 0.25); // +25%
    return {
      promo: {
        id: 'credits_elite_plus_pass_bonus',
        title: 'Credits Elite+ Pass Bonus',
        bonusPercent: 0.25,
        badgeLabel: '+25% Pass Bonus'
      },
      bonusCredits,
      totalCredits: amount + bonusCredits,
    };
  }
  
  if (hasStandard) {
    const bonusCredits = Math.floor(amount * 0.15); // +15%
    return {
      promo: {
        id: 'credits_standard_pass_bonus',
        title: 'Credits Standard Pass Bonus',
        bonusPercent: 0.15,
        badgeLabel: '+15% Pass Bonus'
      },
      bonusCredits,
      totalCredits: amount + bonusCredits,
    };
  }
  
  // Fallback: Prüfe zeitbasierte Promotions
  for (const promo of SHOP_PROMOTIONS) {
    if (promo.type !== 'credit_bonus') continue;
    const start = new Date(promo.startDate);
    const end = new Date(promo.endDate);
    if (now < start || now > end) continue;
    if (!isUserEligible(promo, userHighestVIP)) continue;
    // Min/Max Credits prüfen
    if (typeof promo.minCredits === 'number' && amount < promo.minCredits) continue;
    if (typeof promo.maxCredits === 'number' && amount > promo.maxCredits) continue;
    const bonusCredits = Math.floor(amount * (promo.bonusPercent || 0));
    return {
      promo,
      bonusCredits,
      totalCredits: amount + bonusCredits,
    };
  }
  return null;
};

// Berechnet den Credits-Ausgeben-Rabatt basierend auf Credits-Pässen
// Gibt den Rabatt-Prozentsatz zurück (0.05 = 5%, 0.15 = 15%, etc.)
export const getCreditsSpendingDiscount = (userLicenses = [], now = new Date()) => {
  // Prüfe Elite+ Pass (35%)
  const hasElitePlus = userLicenses.some(l => 
    (typeof l === 'string' && l === 'credits_elite_plus_pass') ||
    (typeof l === 'object' && l.id === 'credits_elite_plus_pass' && (!l.expiresAt || new Date(l.expiresAt) > now))
  );
  if (hasElitePlus) return 0.35;
  
  // Prüfe Standard Pass (25%)
  const hasStandard = userLicenses.some(l => 
    (typeof l === 'string' && l === 'credits_standard_pass') ||
    (typeof l === 'object' && l.id === 'credits_standard_pass' && (!l.expiresAt || new Date(l.expiresAt) > now))
  );
  if (hasStandard) return 0.25;
  
  // Prüfe Basic Pass (15%)
  const hasBasic = userLicenses.some(l => 
    (typeof l === 'string' && l === 'credits_basic_pass') ||
    (typeof l === 'object' && l.id === 'credits_basic_pass' && (!l.expiresAt || new Date(l.expiresAt) > now))
  );
  if (hasBasic) return 0.15;
  
  // Prüfe Free Pass (5%)
  const hasFree = userLicenses.some(l => 
    (typeof l === 'string' && l === 'credits_free_pass') ||
    (typeof l === 'object' && l.id === 'credits_free_pass' && (!l.expiresAt || new Date(l.expiresAt) > now))
  );
  if (hasFree) return 0.05;
  
  return 0; // Kein Rabatt
};

// Countdown-Berechnung: Liefert { days, hours, minutes, expired }
export const getPromoCountdown = (promo, now = new Date()) => {
  if (!promo) return null;
  const end = new Date(promo.endDate);
  const diff = end.getTime() - now.getTime();
  if (diff <= 0) return { days: 0, hours: 0, minutes: 0, expired: true };
  const days = Math.floor(diff / (1000 * 60 * 60 * 24));
  const hours = Math.floor((diff % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
  const minutes = Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60));
  return { days, hours, minutes, expired: false };
};
