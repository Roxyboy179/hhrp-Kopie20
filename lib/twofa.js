...
export async function activateTwoFa(supabaseUserId, code) {
  ...
  delete nextMeta.twofa_pending_secret;
  delete nextMeta.twofa_pending_at;
...
export async function deactivateTwoFa(supabaseUserId) {
  ...
  const meta = userData?.user?.user_metadata || {};
  const nextMeta = { ...meta };
  delete nextMeta.twofa_enabled;
  delete nextMeta.twofa_secret;
  delete nextMeta.twofa_backup_codes;
  delete nextMeta.twofa_enabled_at;
  delete nextMeta.twofa_pending_secret;
  delete nextMeta.twofa_pending_at;
  const { error } = await supabaseAdmin.auth.admin.updateUserById(supabaseUserId, {
    user_metadata: nextMeta,
