...
export async function getAdminAccountByCredentials(mitarbeiterNummer, email, password) {
  console.log('[DEBUG] Looking for account:', { mitarbeiterNummer, email });
  ...
  console.log('[DEBUG] Supabase query result:', error ? `Error: ${error.message}` : `Found: ${account?.mitarbeiter_nummer}`);
  ...
  if (!account) {
    console.log('[DEBUG] No account found');
    return null;
  }

  console.log('[DEBUG] Comparing password...');
  
  // TEMPORÄR: Plain-Text Passwort-Vergleich (für einfaches Setup)
  // TODO: Später auf bcrypt umstellen für Sicherheit
