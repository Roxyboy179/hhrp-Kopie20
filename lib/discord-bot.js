...
export async function sendNewBewerbungNotification(bewerbung, user) {
  console.log('[Discord] === NEUE BEWERBUNG NOTIFICATION ===');
  console.log('[Discord] Bot Token vorhanden:', !!DISCORD_BOT_TOKEN);
  console.log('[Discord] Channel ID:', DISCORD_CHANNEL_ID);
  
  if (!DISCORD_BOT_TOKEN || !DISCORD_CHANNEL_ID) {
    console.warn('[Discord] ❌ Bot Token oder Channel ID fehlt');
    console.warn('[Discord] DISCORD_BOT_TOKEN:', DISCORD_BOT_TOKEN ? 'gesetzt' : 'FEHLT');
    console.warn('[Discord] DISCORD_CHANNEL_ID:', DISCORD_CHANNEL_ID || 'FEHLT');
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('[Discord] ❌ Fehler beim Senden:', response.status, error);
      console.error('[Discord] Response Status:', response.status);
      console.error('[Discord] Response Text:', error);
    } else {
      console.log('[Discord] ✅ Neue Bewerbung gesendet an Kanal');
    }
  } catch (error) {
    console.error('[Discord] ❌ Exception:', error);
    console.error('[Discord] Error Details:', error.message, error.stack);
  }
}
...
export async function sendStatusUpdateDM(userId, status, bewerbungId) {
  console.log('[Discord] === STATUS UPDATE DM ===');
  ...
  try {
    ...
    if (!messageResponse.ok) {
      ...
      console.error('[Discord] ❌ Fehler beim Senden der DM:', messageResponse.status, error);
    } else {
