...
export async function incrementFailedAttempts(email) {
  ...
  try {
    // 1. Supabase Auth User holen (unabhängig von Discord-Link!)
    ...
    // 4. Falls Link existiert, auch dort aktualisieren (für Konsistenz)
    const link = await getLinkByEmail(email);
    if (link) {
      await supabaseAdmin
        .from('user_auth_links')
...
export async function resetFailedAttempts(email) {
  ...
  try {
    // 1. Supabase Auth User holen
    const { data: users } = await supabaseAdmin.auth.admin.listUsers();
    const user = users.users.find(u => u.email?.toLowerCase() === email.toLowerCase());
    
    if (user) {
      // 2. User Metadata zurücksetzen
    ...
    // 3. Falls Link existiert, auch dort zurücksetzen
    const link = await getLinkByEmail(email);
    if (link) {
      await supabaseAdmin
        .from('user_auth_links')
...
export async function checkIfAccountLocked(email) {
  ...
  try {
    ...
    return !!user.user_metadata?.locked_at;
  } catch (error) {
