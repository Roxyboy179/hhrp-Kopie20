...
export async function verifyReauthOtp(email, code) {
  ...
  if (new Date(meta.reauth_otp_expires_at).getTime() < Date.now()) {
    // OTP abgelaufen → räumen wir auf (null statt delete, da Supabase merged)
    const supabaseAdmin = await getSupabaseAdmin();
    const next = {
      ...meta,
      reauth_otp_hash: null,
      reauth_otp_expires_at: null,
      reauth_otp_attempts: null,
    };
  ...
  // null statt delete, da Supabase user_metadata merged
  const next = {
    ...meta,
    reauth_otp_hash: null,
    reauth_otp_expires_at: null,
    reauth_otp_attempts: null,
    reauth_last_sent_at: null,
  };
