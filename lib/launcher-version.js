...
const INSTALLER_FILE = path.join(
  process.cwd(),
  'data',
  'downloads',
  'hhrp-launcher-setup.exe'
);
...
export function getLauncherInstallerPath() {
...
export function isLauncherInstallerAvailable() {
  return fs.existsSync(INSTALLER_FILE);
}
...
export function getLauncherInstallerFileStats() {
  if (!isLauncherInstallerAvailable()) return null;
  const stat = fs.statSync(INSTALLER_FILE);
  return {
    size: stat.size,
    sizeMB: Math.round((stat.size / (1024 * 1024)) * 10) / 10,
    updatedAt: stat.mtime.toISOString(),
