...
export function KrediteDetailView({ userData, onRefresh }) {
  ...
  const sortedKredite = [...kredite].sort((a, b) => {
    const dateA = new Date(a.beantragtAm || a.genehmigtAm || 0);
    const dateB = new Date(b.beantragtAm || b.genehmigtAm || 0);
    return dateB - dateA; // Neueste zuerst
  });
  
  const aktiveKredite = sortedKredite.filter(k => k.status === 'aktiv');
  const pendingKredite = sortedKredite.filter(k => k.status === 'pending');
  const abgeschlosseneKredite = sortedKredite.filter(k => k.status === 'abgeschlossen');
  
  const totalSchulden = aktiveKredite.reduce((sum, k) => sum + (k.rueckzahlungsBetrag || 0), 0);
  
  // State für "Weitere Kredite anzeigen"
  const [showAbgeschlossene, setShowAbgeschlossene] = useState(false);
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-bold text-white flex items-center gap-2">
              ...
              Abgeschlossene Kredite ({abgeschlosseneKredite.length})
            </h3>
            <Button
              onClick={() => setShowAbgeschlossene(!showAbgeschlossene)}
              variant="outline"
              className="rounded-xl"
              style={{
                background: showAbgeschlossene ? 'rgba(34, 197, 94, 0.1)' : 'transparent',
          ...
          {showAbgeschlossene && (
            <div className="grid gap-4">
              {abgeschlosseneKredite.map(renderKreditCard)}
            </div>
          )}
        </div>
      )}
